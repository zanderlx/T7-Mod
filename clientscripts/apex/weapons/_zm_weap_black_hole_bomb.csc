...
include_weapon_for_level()
{
	...
	visionset_register_info("bhb_vision", "zombie_black_hole", 10, 2, 1, false);
...
black_hole_on_player_spawned( int_local_client_num )
{
	...
	for( i = 0 ; i < players.size; i++ )
	{
		// set the parameters for checking black hole dist unless they are already defined
	...
	visionset_deactivate(int_local_client_num, "bhb_vision");
}
...
black_hole_visionset_switch( str_switch, flt_transition_time, int_local_client_num )
{
	...
	switch( str_switch )
	{
		case "inside_bh":
			...
			if( self IsSpectating() ) // if in spectator mode don't take on the vision set otherwise it will not clear
			{
				return;
			}

			visionset_activate(int_local_client_num, "bhb_vision");
