...
include_weapon_for_level()
{
	...
	level.tesla_lightning_params = maps\apex\_zm_lightning_chain::create_lightning_chain_params(
		5, // max_arcs
...
{
	return is_tesla_gun(weapon) && maps\apex\_zm_lightning_chain::is_lightning_chain_damage(mod);
}
...
play_tesla_sound(emotion)
{
	...
	if(is_true(level.one_emo_at_a_time))
	{
		...
		org LinkTo(self);
		org PlaySound(emotion, str_notify);
...
tesla_zombie_damage_response(mod, hit_location, hit_origin, player, amount)
{
	...
	if(is_tesla_damage(mod, weapon))
	{
		self thread maps\apex\_zm_lightning_chain::damage_init(player, level.tesla_lightning_params);
		return true;
	}
	return false;
}