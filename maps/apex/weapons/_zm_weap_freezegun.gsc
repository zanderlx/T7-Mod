...
include_weapon_for_level()
{
	...
	maps\_zombiemode_spawner::register_zombie_death_animscript_callback(::freezegun_death_animscript);
	maps\_zombiemode_spawner::add_custom_zombie_should_gib(::is_freezegun_damage);
	maps\_zombiemode_spawner::register_zombie_damage_callback(::freezegun_zombie_damage_callback);
	maps\_zombiemode_spawner::register_zombie_death_event_callback(::freezegun_zombie_death_event);

	level thread freezegun_on_player_connect();

	level._zm_freezegun_set_extremity_fx = ::freezegun_set_extremity_damage_fx;
	level._zm_freezegun_set_torso_fx = ::freezegun_set_torso_damage_fx;
	level._zm_freezegun_clear_extremity_fx = ::freezegun_clear_extremity_damage_fx;
	level._zm_freezegun_clear_torso_fx = ::freezegun_clear_torso_damage_fx;
}

freezegun_zombie_death_event()
{
	// this gets called before the freezegun gets a chance to set freezegun_death, so we check whether it will do it
	if ( !self should_do_freezegun_death( self.damagemod ) )
	{
	...
	single_thread(self, level._zm_freezegun_clear_extremity_fx);
	single_thread(self, level._zm_freezegun_clear_torso_fx);
}

freezegun_zombie_damage_callback(mod, hit_location, hit_origin, player, amount)
{
...
freezegun_damage_response( player, amount )
{
	...
	single_thread(self, level._zm_freezegun_set_extremity_fx);
}
...
freezegun_death( hit_location, hit_origin, player )
{
	...
	single_thread(self, level._zm_freezegun_set_extremity_fx);
	single_thread(self, level._zm_freezegun_set_torso_fx);
