...
powerup_hack(hacker)
{
	...
	if(IsDefined(self.powerup.zombie_grabbable) && self.powerup.zombie_grabbable)
	{
		...
		{
			self.powerup maps\apex\_zm_powerups::powerup_setup( "full_ammo" );

			self.powerup thread maps\apex\_zm_powerups::powerup_timeout();
			self.powerup thread maps\apex\_zm_powerups::powerup_wobble();
			self.powerup thread maps\apex\_zm_powerups::powerup_grab();
		}
	}
	else if(self.powerup.powerup_name == "full_ammo")
	{
	  self.powerup maps\apex\_zm_powerups::powerup_setup("fire_sale");
	}
	else
	{
	  self.powerup maps\apex\_zm_powerups::powerup_setup("full_ammo");
