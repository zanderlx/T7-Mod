...
sickle_think(cost)
{
	...
	for( ;; )
	{
		...
		if( player HasWeapon( "sickle_knife_zm" ) || player has_powerup_weapon() )
		{
			wait(0.1);
			continue;
		}
		...
		if( is_placeable_mine( current_weapon ) || player has_powerup_weapon() )
		{
			wait(0.1);
			continue;
		}
...
//		player_has_sickle = player HasPerk( "specialty_altmelee" );
		...
		if( !player_has_sickle )
		{
			...
			if( player.score >= cost )
			{
				...
				player maps\apex\_zm_weapons::check_collector_achievement( "sickle_knife_zm" );
...
do_sickle_flourish_end( gun )
{
	...
	else if ( self HasWeapon( "knife_ballistic_upgraded_zm" ) )
	{
		...
		self GiveWeapon( "knife_ballistic_sickle_upgraded_zm", 0, self maps\apex\_zm_weapons::get_pack_a_punch_weapon_options( "knife_ballistic_sickle_upgraded_zm" ) );
	...
	else if ( gun == "knife_zm" ) // if all they had was the knife, we need to switch them to the sickle
	{
		self SwitchToWeapon( "sickle_knife_zm" );
