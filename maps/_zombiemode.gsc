...
#/
		...
		{
			if ( maps\_zm_weapons::is_weapon_included( primaryWeapons[i] ) || maps\_zm_weapons::is_weapon_upgraded( primaryWeapons[i] ) )
			{
...
round_think()
{
	for( ;; )
	{
		...
		maps\_zm_powerups::powerup_round_start();
...
player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	if(self maps\_zm_utility::has_perk("chugabud"))
	{
		if(isdefined(level.chugabud_laststand))
		{
			self thread [[level.chugabud_laststand]]();
			return 0;
		}
	}
