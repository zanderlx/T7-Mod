...
teleporter_init()
{
	...
	flag_init("teleporter_linked", false);
	flag_init("core_linked", false);
	SetDvar("theaterAftereffectOverride", "-1");
	poi1 = GetEnt("teleporter_poi1", "targetname");
	poi2 = GetEnt("teleporter_poi2", "targetname");
	trigger = GetEnt("trigger_teleport_pad_0", "targetname");
	pad = GetEnt(trigger.target, "targetname");
	trigger trigger_off();
	pad trigger_off();

	if(maps\apex\_utility::is_solo_game())
	{
	...
	if(!isdefined(level.teleport_ae_funcs))
		level.teleport_ae_funcs = [];
	if(!IsSplitScreen())
	...
	maps\apex\_zm_power::add_powerable(false, ::teleporter_power_on, ::teleporter_power_off);
	thread theater_fly_me_to_the_moon_init();
	level maps\apex\_utility::levelNotify("pack_clock_start");
}
...
teleporter_power_off()
{
	...
	level notify(trigger_name + "_power_off");
...
teleport_core_think( index )
{
	...
	level endon(trigger_name + "_power_off");
	active = false;
	user = undefined;
	...
	if ( isdefined( trigger ) )
	{
		while ( !active )
		{
			...
			if ( is_player_valid( user ) && user.score >= level.teleport_cost )
			{
				...
				trigger thread teleport_player_thread(index);
				trigger waittill("teleport_done");
...
player_teleporting( index )
{
	...
	// players are forced back to level
	if ( maps\apex\_zm_power::is_power_on() && randomint( 100 ) > 24 && !IsDefined( level.eeroomsinuse ) ) // ww: chaning the frequency to 75%
	{
		loc = "eerooms";
