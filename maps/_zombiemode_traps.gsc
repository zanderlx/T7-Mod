...
trap_init()
{
	...
	if ( IsDefined(self.script_noteworthy) )
	{
		...
		if(isdefined(level._zombiemode_trap_deactivate_funcs) && isdefined(level._zombiemode_trap_deactivate_funcs[self._trap_type]))
			self._trap_deactivate_func = level._zombiemode_trap_deactivate_funcs[self.trap_type];
		else
		{
			switch(self.script_noteworthy)
			{
				case "rotating":
					self._trap_deactivate_func = ::trap_deactivate_rotating;
					break;
				case "electric":
					self._trap_deactivate_func = ::trap_deactivate_electric;
					break;
				case "flipper":
					self._trap_deactivate_func = ::trap_deactivate_flipper;
					break;
				case "fire":
				default:
					self._trap_deactivate_func = ::trap_deactivate_fire;
					break;
			}
		}
...
//	self.use_this_angle = getstruct ("use_this_angle", "script_string");
	...
	self thread trap_activate_think();
...
trap_power_off()
{
	trap = self.trap;
	trap trap_disable();
	array_thread(trap._trap_use_trigs, ::trigger_on); // need to reenable triggers as trap_disable() disables them
	trap trap_set_string(&"ZOMBIE_NEED_POWER");
	trap trap_lights_red();
}
...
trap_use_think( trap )
{
	while(1)
	{
		...
		if( is_player_valid( who ) && !trap._trap_in_use )
		{
			...
			single_thread(trap, trap._trap_deactivate_func);
